package com.andan.contohlocaldb
...
import com.andan.contohlocaldb.adapter.NoteAdapter
    ...
    val resultLauncher: ActivityResultLauncher<Intent> = registerForActivityResult(
        ...
        if (result.data != null) {
            ...
            when (result.resultCode) {
                NoteUpdateActivity.RESULT_ADD -> {
                    val note = result.data?.getParcelableExtra<Note>(NoteUpdateActivity.EXTRA_NOTE) as Note
                ...
                NoteUpdateActivity.RESULT_UPDATE -> {
                    val note = result.data?.getParcelableExtra<Note>(NoteUpdateActivity.EXTRA_NOTE) as Note
                    val position = result?.data?.getIntExtra(NoteUpdateActivity.EXTRA_POSITION, 0) as Int
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = NoteAdapter(object : NoteAdapter.OnItemClickCallback {
            override fun onItemClicked(selectedNote: Note?, position: Int?) {
                ...
                intent.putExtra(NoteUpdateActivity.EXTRA_NOTE, selectedNote)
